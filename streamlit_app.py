...
def load_model():
    try:
        ...
        st.info("Please make sure the model files are in the correct location.")
        return None, None
...
def main():
    ...
    if model is None:
        ...
        **Model not loaded. Please ensure:**
        1. You have run the Jupyter notebook to train the model  
        2. The model files (`best_fraud_model.pkl` and `scaler.pkl`) are in the correct location  
        3. The files are in the same directory as this app or in a 'models' subfolder
        """)
        return
